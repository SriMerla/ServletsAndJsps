...
public class ReadParameters {
	 ...
	 * (String)paramNames.nextElement(); out.print("<tr><td>" + paramName +
	 * "</td>\n<td>"); String[] paramValues = request.getParameterValues(paramName);
	 * 
	 * // Read single valued data if (paramValues.length == 1) { String paramValue =
	 * paramValues[0]; if (paramValue.length() == 0) out.println("<i>No Value</i>");
	 * else out.println(paramValue); } else { // Read multiple valued data
	 * out.println("<ul>");
	 * 
	 * for(int i = 0; i < paramValues.length; i++) { out.println("<li>" +
	 * paramValues[i]); } out.println("</ul>"); } }
	 * out.println("</tr>\n</table>\n</body></html>"); }
	 * 
